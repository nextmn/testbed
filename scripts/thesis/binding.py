#!/usr/bin/env python3
'''Create plot for thesis/binding scenario'''
...
def plot(arguments: argparse.Namespace):
    '''Write plot'''
    res = []
    with open(arguments.slice1, 'r', encoding='utf8') as ping:
        res.append({'tsp': [], 'pqt': []})
    ...
    with open(arguments.slice2, 'r', encoding='utf8') as ping:
        res.append({'tsp': [], 'pqt': []})
        for i, line in enumerate(ping):
            if 'time=' in line:
    ...
    #axplt.xaxis.set_label_coords(1.13, -0.025)
...
if __name__ == '__main__':
    ...
    parser.add_argument('slice1', type=pathlib.Path,
            help='ping result file')
    parser.add_argument('slice2', type=pathlib.Path,
            help='ping result file')
